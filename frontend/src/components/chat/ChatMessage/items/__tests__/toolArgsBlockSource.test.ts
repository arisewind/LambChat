...
test("tool argument blocks share detail and compact wrappers", () => {
  ...
  expect(source).toMatch(
    /detail:\s*"tool-args-block group\/args relative flex items-center gap-2 px-3 py-2 rounded-\[var\(--radius-sm\)\] bg-theme-bg-card text-14 text-theme-text-tertiary font-mono/,
  );
  expect(source).toMatch(
    /compact:\s*"tool-args-block group\/args relative flex items-center gap-2 mb-2 px-2 py-1\.5 rounded-\[var\(--radius-sm\)\] bg-theme-bg-card text-12 text-theme-text-tertiary font-mono/,
  );
