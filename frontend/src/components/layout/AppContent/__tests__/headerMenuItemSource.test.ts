...
test("header overflow menu shares item and icon wrappers", () => {
  ...
  expect(
    source.match(
      /flex w-full items-center gap-3 px-3 py-2\.5 text-left text-14 transition-colors text-\[var\(--theme-text-secondary\)\] hover:text-\[var\(--theme-text\)\] hover:bg-\[var\(--theme-bg-subtle\)\]/g,
    )?.length,
  ).toBe(1);
  expect(
